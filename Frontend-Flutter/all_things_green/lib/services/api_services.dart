...
class ApiManager extends GetConnect {
  Future<String> getDistrict(double lat, double lng) async {
    try {
      final response = await http.get(Uri.parse('https://api.opencagedata.com/geocode/v1/json?key=${AppConstants.openCageAPIKey}&q=$lat+$lng'));

if (response.statusCode == 200) {
  final json = jsonDecode(response.body);
  final district = json['results'][0]['components']['county']; // get the district name from the API response
  ...
  Future<String> getOldDistrict(String currentDistrict) async {
    try {
      String dataPortalUrl = "https://api.data.gov.in/resource/cd5e209a-f8d7-4449-bbe9-d2a2af2500d4?api-key=${AppConstants.dataGovAPIKey}&format=json&filters[districtname2010]=$currentDistrict";
